...
const Appointment: React.FC = () => {
        ...
        return (
            <AnimatedPage>
                <div className="text-center py-20">
                    <h2 className="text-2xl font-serif">No service selected.</h2>
                    <p className="mt-2">Please select a service first.</p>
                    <button onClick={() => navigate('/services')} className="mt-4 px-6 py-2 bg-accent-color text-white font-bold rounded-lg">Go to Services</button>
                </div>
            </AnimatedPage>
        );
    }
    ...
    if (booked) {
        return (
            <div className="relative w-full h-screen flex items-center justify-center overflow-hidden bg-[#FFFBF5]">
                <Confetti />
                <motion.div
                ...
                >
                    <h1 className="text-4xl font-serif text-[#4E443C]">Booking Confirmed!</h1>
                    <p className="mt-2 text-lg text-[#4E443C]/80">Redirecting to payment...</p>
                </motion.div>
            </div>
        )
    }
    
    const inputStyle = "w-full px-4 py-3 bg-[#2a2a2a] border border-gray-600 rounded-lg focus:outline-none focus:ring-2 ring-accent-color text-gray-300 placeholder:text-gray-500";


    return (
        <AnimatedPage>
            <div className="min-h-screen flex items-center justify-center dark-theme-bg py-12 px-4 sm:px-6 lg:px-8">
                <div className="w-full max-w-2xl p-10 space-y-8 bg-[#1F1F1F] rounded-xl shadow-lg text-white">
                    <div className="text-left">
                        <h1 className="text-3xl font-serif font-bold">Appointment Form</h1>
                        <p className="mt-2 text-gray-400">
                            You're booking: 
                            <span className="font-bold text-[#D97706]">
                                {totalItems > 1 ? ` ${totalItems} services` : ` ${itemsToBook[0].name}`}
                            </span> 
                             for a total of <span className="font-bold text-[#D97706]">Rs{totalPrice}</span>.
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <select name="serviceName" required className={inputStyle} disabled>
                                <option>{totalItems > 1 ? `${totalItems} services selected` : itemsToBook[0].name}</option>
                            </select>
