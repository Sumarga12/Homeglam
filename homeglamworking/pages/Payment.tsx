                        ...
                        <div className="space-y-4">
                            {paymentOptions.map(option => (
                                <div
                                    ...
                                    className={`p-4 border rounded-lg flex items-center justify-between cursor-pointer transition-all ${selectedPayment === option.name ? 'border-[#D97706] ring-2 ring-[#D97706]' : 'border-gray-300 hover:border-gray-400'}`}
                                >
                                    ...
                                    {selectedPayment === option.name && <CheckCircleIcon className="w-6 h-6 text-[#D97706]" />}
                                </div>
                            ))}
                        </div>
                        
                        <AnimatePresence>
                        {selectedPayment === 'Visa / MasterCard' && <VisaForm />}
                        {selectedPayment === 'PayPal' && <PayPalForm />}
                        {selectedPayment === 'eSewa' && <WalletForm walletName="eSewa" />}
                        {selectedPayment === 'Khalti' && <WalletForm walletName="Khalti" />}
                        </AnimatePresence>
                        
                        <motion.button
                            onClick={handlePayment}
                            disabled={!selectedPayment || processing}
                            className="w-full mt-8 py-3 px-6 bg-accent-color text-white font-bold rounded-lg shadow-lg hover:bg-orange-700 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
