...
const NavItem: React.FC<{ to: string; children: React.ReactNode }> = ({ to, children }) => (
    <NavLink
        to={to}
        className={({ isActive }) =>
            `relative text-sm font-medium transition-colors text-[#4E443C] hover:text-[#D97706] ${isActive ? 'text-[#D97706]' : ''}`
        }
    >
        {({ isActive }) => (
            <>
                ...
                {isActive && (
                    <motion.div
                        className="absolute bottom-[-6px] left-0 right-0 h-[3px] bg-[#D97706]"
                        layoutId="underline"
...
const Header: React.FC = () => {
    ...
    return (
        <header className="sticky top-0 z-50 w-full bg-[#FFFBF5]/80 backdrop-blur-sm border-b border-[#4E443C]/10">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    {/* Logo */}
                     <div className="flex-1 flex justify-start">
                        <Link to="/" className="text-3xl font-serif font-bold text-[#4E443C]">
                            HomeGlam
                        </Link>
                    </div>
                    ...
                    <div className="flex-1 flex justify-end items-center space-x-4">
                       <AnimatePresence mode="wait">
                         {!isSearchOpen && (
                            ...
                            >
                                <Link to="/cart" className="relative">
                                    <ShoppingBagIcon className="h-6 w-6 text-[#4E443C] hover:text-[#D97706] cursor-pointer" />
                                    {cartCount > 0 && (
                                ...
                                {user ? (
                                    ...
                                    <Link to="/login">
                                        <UserCircleIcon className="h-8 w-8 text-[#4E443C] hover:text-[#D97706]" />
                                    </Link>
                                )}
                            </motion.div>
                         )}
                        ...
                        <button onClick={() => setIsSearchOpen(!isSearchOpen)} className="transition-colors z-10">
                             {isSearchOpen ? <XMarkIcon className="h-6 w-6 text-[#4E443C] hover:text-[#D97706]" /> : <MagnifyingGlassIcon className="h-6 w-6 text-[#4E443C] hover:text-[#D97706]" />}
                        </button>
                    </div>
                </div>
            </div>
